function createSvg(R = 'S=AX,=title,A=[+AX-AX-AX]-AX+AX+AX-,F=,X=F+F+F+FFF-F-F-F,_a=60,_n=3') {
  ...
  let t0 = performance.now(), [x, y, a, b, q, o] = Array(9).fill(0), d = '', p = 1, i, j, Q = Math.PI / 2;
  ...
  svg.replaceChildren(
    (t => (t.textContent = JSON.stringify(R, Object.keys(R).sort()).replace(/[{}"]/g, ''), t))(C('title')),
    (t => (t.textContent = JSON.stringify(D.stat()), t))(C('desc')),
